...
begin
  ...
  select coalesce(array_agg(id_value::text), array[]::text[])
  into reset_class_text_ids
  from unnest(reset_class_ids) as ids(id_value);

  delete from storage.objects
  where bucket_id = 'simulation-pdfs'
    and (storage.foldername(name))[1] = any(reset_class_text_ids);

  delete from storage.objects
  where bucket_id = 'avatars'
    and (storage.foldername(name))[1] = any(demo_user_text_ids);
