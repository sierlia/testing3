...
create unique index if not exists subcommittees_committee_name_idx on public.subcommittees(committee_id, name);

alter table public.bill_referrals
  add column if not exists subcommittee_id uuid references public.subcommittees(id) on delete set null;

create index if not exists bill_referrals_subcommittee_id_idx on public.bill_referrals(subcommittee_id);
...
create index if not exists subcommittee_members_user_id_idx on public.subcommittee_members(user_id);
